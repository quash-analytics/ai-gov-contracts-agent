 ...
 *                     command that tries to wipe jarvis's runtime data (.jarvis, the
...
// Team Rocket guard: hands off the Pokemon Center (.jarvis runtime data).
// Blocks the exact mistake jarvis's own rules forbid — wiping .jarvis or `rm -rf /`.
const ROCKET = /\brm\b[^\n]*\s(-[rRfF]+\s+)?(\.jarvis(\/|\b)|\/\s|~\/?\s*$)/;
	...
	pi.on("tool_call", async (event) => {
		...
		if (ROCKET.test(command)) {
			return {
				block: true,
				reason:
					"Team Rocket guard: that command targets the Pokemon Center (.jarvis " +
